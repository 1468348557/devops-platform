...
def _machine_review_sql_files(
    ...
    5) DDL 含 create table 时必须是 create table if not exists；
       回滚含 drop table 时必须是 drop table if exists。
    """
    normalized_db_name = (expected_db_name or "").strip()
    phase_hits = {"ddl": 0, "backup": 0, "execute": 0, "rollback": 0}
    create_table_re = re.compile(r"\bcreate\s+table\s+if\s+not\s+exists\b", re.IGNORECASE)
    drop_table_re = re.compile(r"\bdrop\s+table\s+if\s+exists\b", re.IGNORECASE)
    use_db_re = (
        re.compile(
    ...
    for sql_path in selected_paths:
        ...
        if matched_phase == "ddl" and "create table" in lower_content and not create_table_re.search(content):
            return False, f"机器审批不通过：DDL 脚本 {file_name} 创建表必须使用 create table if not exists"
        if matched_phase == "rollback" and "drop table" in lower_content and not drop_table_re.search(content):
            return False, f"机器审批不通过：回滚脚本 {file_name} 必须使用 drop table if exists"
