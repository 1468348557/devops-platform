...
class SqlExecutionOrderTests(TestCase):
    ...
    def test_execute_in_fixed_order_by_file_name(self, mocked_connect, mocked_get_config):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            selected = json.dumps([str(execute), str(rollback), str(backup), str(ddl)], ensure_ascii=False)
            ok, _, _ = execute_sql_request(str(folder), selected)

        self.assertTrue(ok)
        executed_sql = [args[0] for args, _kwargs in cursor.execute.call_args_list]
        self.assertEqual(executed_sql, ["select 22", "select 11", "select 33"])

    @patch("sql_execute.services.GitPlatformConfig.get_solo_safe")
    @patch("sql_execute.services.pymysql.connect")
    def test_fail_before_rollback_then_run_rollback(self, mocked_connect, mocked_get_config):
        config = mocked_get_config.return_value
        config.sql_db_host = "127.0.0.1"
        config.sql_db_name = "demo"
        config.sql_db_user = "demo"
